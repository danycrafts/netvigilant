...
class _SearchScreenState extends State<SearchScreen>
  ...
  Future<void> _fetchApps() async {
    ...
    final cachedAppsJson = prefs.getString('cached_apps');

    if (cachedAppsJson != null) {
      final List<dynamic> cachedAppsList = json.decode(cachedAppsJson);
      final apps = cachedAppsList.map((app) => Application.fromMap(app)).toList();
      if (mounted) {
        setState(() {
          _apps = apps;
          _filteredApps = apps;
      ...
      final apps = await compute(_getApps, null);
      final List<Map<String, dynamic>> appsJson = apps.map((app) => app.toMap()).toList();
      await prefs.setString('cached_apps', json.encode(appsJson));
      if (mounted) {
        setState(() {
          _apps = apps;
          _filteredApps = apps;
  ...
  static Future<List<Application>> _getApps(void _) async {
